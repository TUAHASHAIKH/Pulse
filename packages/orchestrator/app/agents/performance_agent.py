"""
Pulse Orchestrator — Performance Agent
"""
...
async def run(
    ...
    await emit_event("agent_started", {
        "agent": AGENT_NAME,
        "status": "scanning",
        "message": f"Performance Agent scanning {len(changed_files)} file(s)...",
    })

    logger.info(
        f"Performance Agent starting — "
    ...
    try:
        ...
        logger.info(
            f"Performance Agent finished — {len(findings)} findings, "
